...
namespace CitrioN.UI
{
  ...
  public static partial class UIPanelManager
  {
    ...
    {
      var panel = EnabledPanels?.Find(p => typeof(T).IsAssignableFrom(p.GetType()));
      if (panel == null)
      {
        ConsoleLogger.LogWarning($"Could not find UIPanel of type {typeof(T)}");
      }
      return panel as T;
    }
    ...
    {
      var panel = EnabledPanels?.Find(p => typeof(T).IsAssignableFrom(p.GetType()) && p.PanelName == panelName);
      if (panel == null)
      {
        ConsoleLogger.LogWarning($"Could not find UIPanel of type {typeof(T)} with name {panelName}");
      }
      return panel as T;
    }
    ...
    public static AbstractUIPanel GetPanel(Type type)
    {
    ...
    public static List<T> GetPanels<T>() where T : AbstractUIPanel
    {
      List<T> panels = new List<T>();
      EnabledPanels?.ForEach(p =>
      {
        if (p is T panel) { panels.Add(panel); }
      });
    ...
    /// Empty list if no match was found.</returns>
    public static List<AbstractUIPanel> GetPanels(Type type)
    {
      List<AbstractUIPanel> panels = new List<AbstractUIPanel>();
      EnabledPanels?.ForEach(p =>
      {
        if (p.GetType() == type) { panels.Add(p); }
      });
      if (panels == null || panels.Count < 1)
      {
        ConsoleLogger.LogWarning($"Could not find UIPanel of type {type.FullName}");
